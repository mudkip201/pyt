...
def interpret(cc,stck,i,line):
        ...
        if(isinstance(q,list)):
            if(isinstance(qq,list)):
                if(isinstance(qqq,list)):
                    stck.append([pow(int(qqq[j]),int(qq[j]),int(q[j])) for j in range(min(len(q),len(qq),len(qqq)))])
                else:
                    stck.append([pow(int(qqq),int(qq[j]),int(q[j])) for j in range(min(len(q),len(qq)))])
            else:
                if(isinstance(qqq,list)):
                    stck.append([pow(int(qqq[j]),int(qq),int(q[j])) for j in range(min(len(q),len(qqq)))])
                else:
                    stck.append([pow(int(qqq),int(qq),int(x)) for x in q])
        else:
            if(isinstance(qq,list)):
                if(isinstance(qqq,list)):
                    stck.append([pow(int(qqq[j]),int(qq[j]),int(q)) for j in range(min(len(qq),len(qqq)))])
                else:
                    stck.append([pow(int(qqq),int(x),int(q)) for x in qq])
            else:
                if(isinstance(qqq,list)):
                    stck.append([pow(int(x),int(qq),int(q)) for x in qqq])
                else:
                    stck.append([pow(int(qqq),int(qq),int(q))])
    elif cc==u"|":
        q=stck.pop()
        qq=stck.pop()
        if(isinstance(q,np.ndarray)):
    ...
    #stck.append(raw_input())
        ...
        if(isinstance(q,np.ndarray)):
            q=q.tolist()
        if(isinstance(q,list)):
            stck.append([2**int(x).bit_length()-int(x) for x in q])
        else:
            stck.append(2**int(q.bit_length())-int(q))
    elif cc==u"%":
        q=stck.pop()
        qq=stck.pop()
        if(isinstance(qq,np.ndarray)):
        ...
        if(isinstance(stck[-2],np.ndarray) and isinstance(stck[-1],np.ndarray)):
           q=stck.pop()
           stck.append(np.divide(stck.pop(),q))
        elif(isinstance(stck[-2],np.ndarray) and isinstance(stck[-1],list)):
            q=np.array(stck.pop())
            stck.append(np.divide(stck.pop(),q).tolist())
        elif(isinstance(stck[-2],list) and isinstance(stck[-1],np.ndarray)):
            q=stck.pop()
            stck.append(np.divide(np.array(stck.pop()),q))
        elif(isinstance(stck[-2],list) and isinstance(stck[-1],list)):
            q=stck.pop()
        ...
        stck.append(primeFactors(int(stck.pop())))
    elif cc==u"₫":
        ...
        if(isinstance(qq,np.ndarray)):
            qq=qq.tolist()
        if(isinstance(qq,list)):
            stck.append([log(x,q) for x in qq])
        else:
            stck.append(log(qq,q))
    elif cc==u"Ĺ":
        q=stck.pop()
        qq=stck.pop()
        if(isinstance(q,np.ndarray)):
        ...
        if(isinstance(qq,list)):
            if(isinstance(q,list)):
                print([base(int(qq[j]),int(q[j])) for j in range(min(len(qq),len(q)))])
            else:
                print([base(int(x),int(q)) for x in qq ])
        else:
            if(isinstance(q,list)):
                print([base(int(qq),int(x)) for x in q])
            else:
                print(base(int(qq),int(q)))
    elif cc==u"ƥ":
        print(unicode(stck.pop()))
    elif cc==u"ṗ":
        q=stck.pop()
